...
grid_points_file = os.path.join(dem_dir, "AgERA5_GridPoints.csv")  # Using the grid points CSV with lon before lat
...
grid_points['elevation'] = [
    get_elevation(lat, lon, dem_data, dem_transform, dem_bounds) for lon, lat in zip(grid_points['longitude'], grid_points['latitude'])
]
