...
def download_VP(year, month, base_dir):
    # Define the download directory based on the base_dir
    download_directory = os.path.join(base_dir, "VP")
    ...
    output_file = os.path.join(download_directory, f"AgERA5_VP_{month_name}_{year}.zip")

    # Download the data using the CDS API
    client = cdsapi.Client()
    client.retrieve(dataset, request).download(output_file)

    print(f"Vapour Pressure data successfully downloaded to {output_file}")