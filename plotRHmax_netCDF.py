# -*- coding: utf-8 -*-
"""
Script to plot Maximum Relative Humidity (RHmax) from netCDF files for a specific date.
...
def load_rhmax_data(file_path, variable_name):
    # Load RHmax data from NetCDF file
    dataset = nc.Dataset(file_path)
    lon = dataset.variables['lon'][:]
    lat = dataset.variables['lat'][:]
    rhmax = dataset.variables[variable_name][:]
    return lon, lat, rhmax

def plot_rhmax(lon, lat, rhmax_data):
    ...
    rhmax_slice = rhmax_data[0, :, :]
    masked_rhmax = np.ma.masked_invalid(rhmax_slice)

    # Plot RH
    contour = ax.contourf(
        lon, lat, masked_rhmax,
        levels=bounds,
    ...
    cbar.set_label('Maximum Relative Humidity (%)')
...
if __name__ == "__main__":
    rhmax_file = fr'C:\AgERA5\RH\RHmax-2m_C3S-glob-agric_AgERA5_{YYYYMMDD}_final-v1.1.area-subset.-20.15.-35.35.nc'
    rhmax_variable = 'RHmax'

    lon, lat, rhmax_data = load_rhmax_data(rhmax_file, rhmax_variable)
    plot_rhmax(lon, lat, rhmax_data)
