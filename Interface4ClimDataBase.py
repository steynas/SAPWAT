...
def display_basic_stats(climate_data):
    """
    Display basic statistics for the loaded climate data.
    Parameters:
    - climate_data (DataFrame): The loaded climate data for a specific grid point.
    """
    print("\nBasic Statistics for the Selected Climate Data:")
    print("------------------------------------------------")
    print(climate_data.describe())  # Shows count, mean, std, min, 25%, 50%, 75%, max for numerical columns
    print("------------------------------------------------")
    print(f"Date range: {climate_data['Date'].min()} to {climate_data['Date'].max()}")
    print(f"Total number of records: {len(climate_data)}")
    print(f"Columns available: {', '.join(climate_data.columns)}")

# Function to handle the click event on the map
def onclick(event, ax, button):
    global selected_lon, selected_lat, popup_text, instruction_text, selected_marker, distance_text
    ...
    if is_point_on_land(lon, lat):
        ...
        # Add a marker to the selected point and display its coordinates
        ...
        # Find the closest gridpoint but don't yet proceed with loading
...
def on_button_clicked(event):
    ...
    if selected_lon is not None and selected_lat is not None:
        ...
        # Find the closest gridpoint after the coordinates are confirmed
        ...
        # Calculate and display the distance to the closest gridpoint
        ...
        # Construct the filename and path using the closest gridpoint
        closest_lat = closest_gridpoint['latitude']
        closest_lon = closest_gridpoint['longitude']
        lat_str = f"{closest_lat:.1f}"
        lon_str = f"{closest_lon:.1f}"
        clim_file = f"ClimateTimeSeries_AgERA5_{lat_str}_{lon_str}.csv"
        clim_path = os.path.join(clim_dir, clim_file)
        
        # Check if the file exists and load the data
        if os.path.exists(clim_path):
            print(f"Loading climate data from: {clim_file}")
            try:
                climate_data = pd.read_csv(clim_path)
                print(f"Data loaded successfully from {clim_file}")
                # Call the new function to display basic stats
                display_basic_stats(climate_data)
            except Exception as e:
                print(f"Error reading the file: {e}")
        else:
            print(f"File {clim_file} not found in the ClimDataBase directory.")
        
        plt.close()  # Close the map when the coordinates are confirmed
    else:
        print("No valid coordinates selected. Please try again.")
...
clim_dir = os.path.join(base_dir, "ClimDataBase")
